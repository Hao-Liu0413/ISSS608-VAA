...
popdata <- read_csv("Hands-on_Ex08/data/aspatial/respopagesextod2011to2020.csv")
```
...
```{r}
write_rds(mpsz_pop2020, "Hands-on_E08/data/rds/mpszpop2020.rds")
```
...
# 1) switch to static plotting mode
tmap_mode("plot")

# 2) build the map, relying on tmap’s style parameters
tm_shape(mpsz_pop2020) +
  tm_polygons(
    "DEPENDENCY",               # directly map the field
    style    = "quantile",      # classification scheme
    n        = 5,               # number of bins
    palette  = "Blues",         # single-brewer palette
    title    = "Dependency ratio"
  ) +
  tm_borders(alpha = 0.5) +
  tm_compass(type = "8star", size = 2) +
  tm_grid(alpha = 0.2) +
  tm_title("Distribution of Dependency Ratio\nby planning sub-zone") +
  tm_credits(
    "Source: URA planning sub-zone boundaries\nand DOS population data",
    position = c("left", "bottom")
  )
# (no assignment—this map object is printed automatically)
```
